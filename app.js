/* v0.4: Twilight & Moon Boost + Dynamic CUTOFF (0–24 eksen)
   - Astronomy: sunrise, sunset, moonrise, moonset
   - Score = WeatherGaussian + TwilightBoost + MoonBoost (cap 100)
   - CUTOFF = p70 of the day (bounded 55..85)
*/
...
// Boost weights (istersen ayarlayabilirsin)
const TWILIGHT_BOOST_MAX = 18;   // sunrise/sunset çevresi (±45–60 dk) tepe katkı
const TWILIGHT_SIGMA_H   = 0.8;  // saat cinsinden sigma
const MOON_BOOST_MAX     = 10;   // moonrise/moonset çevresi katkı
...
function computeDaily(forecast, astro){
  ...
  for(let i=0;i<dates.length;i++){
    ...
    const sunrise  = (A.sunrise && A.sunrise[ai])   ? toHour(A.sunrise[ai])   : null;
    const sunset   = (A.sunset && A.sunset[ai])     ? toHour(A.sunset[ai])    : null;
    ...
    // Dinamik eşik: gün içi p70 (55..85 bandı)
    ...
    // Eşik üstü ardışık blokları çıkar
    const blocks = [];
    let cur = [];
    ...
    // Gün skoru: en iyi bloğun ortalaması
